...
| **4** | **Contour Length & Area** | Haversine per-segment polyline length; Planar polygon area using the Shoelace formula | `backend/services/contour_service.py`<br>`backend/utils/geo_utils.py` | **PASS** | Polyline length in meters/km. Enclosed area ($m^2$/$km^2$) computed **only** when `is_closed == True` and vertices $\ge 3$. |
| **5** | **Slope & Terrain Analysis** | Central finite gradient $\arctan \sqrt{dx^2 + dy^2}$ (`np.gradient`); aspect compass directions | `backend/services/terrain_service.py`<br>`frontend/src/components/StatsPanel.tsx` | **PASS** | Slope heatmap PNG overlay ($0^\circ \dots 90^\circ$); aspect cardinal direction ($N, NE, E \dots$). Uses central finite differences. |
...
| **10** | **Runoff Estimation** | Runoff Coefficient Method ($V = P \times A \times C$) | `backend/services/runoff_service.py`<br>`backend/models/runoff_models.py` | **PASS** | $P = \text{mm}/1000 \text{m}$, $A = m^2$. Configurable $C$ presets ($0.15 \dots 0.85$). Labeled as "Estimated Runoff". Distinct from peak discharge $Q = C \cdot i \cdot A$. |
...
| **13** | **Pond Storage Capacity & Stage-Storage Curve** | Raster grid volume integration $V(z) = \sum_{i} \max(0, z - E_i) \Delta A_i$ | `backend/services/pond_service.py`<br>`frontend/src/components/PondInspector.tsx` | **PASS** | 10-step Stage-Storage Curve ($z \to V(z)$ & $A(z)$) rendered in Plotly chart. $V(0) = 0$; monotonic volume increase. |
...
- `test_dem.py`: 8/8 PASSED (elevation range, Perlin fallback non-repeatability check [synthetic engine only], fill NaNs, slope/aspect bounds, pixel size)
- `test_hydrology.py`: 11/11 PASSED (D8 flow direction, flow accumulation bounds, watershed area & perimeter bounds)
- `test_pond_runoff_suitability.py`: 23/23 PASSED (pond depth, volume, surface area, Runoff Coefficient Method runoff, suitability composite ranking)
...
3. **Synthetic Test Notice:** Tests using Perlin noise validate ONLY the synthetic fallback generator's determinism and non-repeatability. They do not validate real-world satellite DEM acquisition.
