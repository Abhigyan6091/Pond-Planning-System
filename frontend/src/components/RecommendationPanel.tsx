...
import { Star, MapPin, CheckCircle, AlertTriangle, Target, Waves, Mountain, Droplets, BarChart2 } from 'lucide-react';
...
export const RecommendationPanel: React.FC<RecommendationPanelProps> = ({
  ...
  return (
    <DraggablePanel
      ...
      subtitle={`${recommended.lat.toFixed(4)}°N, ${recommended.lng.toFixed(4)}°E`}
    ...
    >
      {/* Score ring + location */}
      <div className="flex items-center gap-3 bg-[#0a0d14]/80 p-2.5 rounded-lg border border-[#1f293d]">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[10px] font-mono text-slate-400 mb-1 flex items-center space-x-1">
            ...
            <span className="truncate">{recommended.lat.toFixed(5)}°N, {recommended.lng.toFixed(5)}°E</span>
          </div>
      ...
      <div className="bg-[#0a0d14]/80 rounded-lg border border-[#1f293d] p-2.5 grid grid-cols-3 gap-2 text-center">
        {[
          ...
          { label: 'Surface', value: `${(recommended.estimated_surface_area_m2 / 10000).toFixed(2)} ha`, Icon: Waves, color: 'text-blue-400' },
          {
            label: 'Volume',
            value:
              recommended.estimated_volume_m3 > 1000
      ...
      {recommended.estimated_runoff_m3 && (
        <div className="bg-blue-900/10 border border-blue-500/20 rounded-lg px-3 py-2 flex items-center justify-between">
          ...
          <div className="text-[9px] text-slate-500 text-right">C = {recommended.runoff_coefficient}</div>
        </div>
      )}
      ...
      <div>
        ...
        <div className="space-y-1 font-mono text-[10px] bg-[#0a0d14]/60 p-2 rounded-lg border border-[#1f293d]">
          <div className="flex justify-between text-slate-300">
            <span>Slope Suitability</span>
            <span className="text-emerald-400 font-bold">
              {recommended.scores.slope_pts ?? (recommended.scores.slope_score * 20).toFixed(1)} / 20
            </span>
          </div>
          <div className="flex justify-between text-slate-300">
            <span>Depression Depth</span>
            <span className="text-cyan-400 font-bold">
              {recommended.scores.depression_pts ?? (recommended.scores.depression_score * 20).toFixed(1)} / 20
            </span>
          </div>
          <div className="flex justify-between text-slate-300">
            <span>Catchment & Flow</span>
            <span className="text-indigo-400 font-bold">
              {recommended.scores.catchment_pts ?? (recommended.scores.catchment_score * 25).toFixed(1)} / 25
            </span>
          </div>
          <div className="flex justify-between text-slate-300">
            <span>Terrain Elevation</span>
            <span className="text-purple-400 font-bold">
              {recommended.scores.elevation_pts ?? (recommended.scores.elevation_score * 15).toFixed(1)} / 15
            </span>
          </div>
          <div className="flex justify-between text-slate-300">
            <span>Rainfall Volume</span>
            <span className="text-blue-400 font-bold">
              {recommended.scores.rainfall_pts ?? (recommended.scores.rainfall_score * 20).toFixed(1)} / 20
            </span>
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            {recommended.suitability_reasons.map((r, i) => (
              <div
                key={i}
                className={`flex items-start space-x-1.5 text-[11px] ${
                  r.startsWith('✓') ? 'text-emerald-400' : 'text-amber-400'
                }`}
              >
                {r.startsWith('✓') ? (
